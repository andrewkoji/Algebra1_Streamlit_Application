...
# Ensure r is consistent as an integer if possible
r = df_geo['Geometric Sequence'][1] // df_geo['Geometric Sequence'][0] if df_geo['Geometric Sequence'][1] % df_geo['Geometric Sequence'][0] == 0 else df_geo['Geometric Sequence'][1] / df_geo['Geometric Sequence'][0]

# Ensure all sequence values are integers if possible
df_geo['Geometric Sequence'] = df_geo['Geometric Sequence'].apply(lambda x: int(x) if x == int(x) else x)

sequence = df_geo.to_dict(orient="records")

# SECTION: Display Header and Formula
st.markdown("<h1 style='text-align: center;'>Geometric Sequences</h1>", unsafe_allow_html=True)
...
with col4:
    st.latex(r"""r""")
    st.markdown("<p style='text-align: center;'>common ratio</p>", unsafe_allow_html=True)
...
with col1:
    n = st.number_input("Enter a value for n:", min_value=1, max_value=len(df_geo['Geometric Sequence']), value=4, step=1, label_visibility="visible")
with col2:
    st.write("")  # Empty space for alignment

# SECTION: Calculate Values for Formula
if "a_1" not in st.session_state:
    st.session_state.a_1 = df_geo['Geometric Sequence'][0]  # Fixed column name
if "r" not in st.session_state:
    st.session_state.r = df_geo['Geometric Sequence'][1] / df_geo['Geometric Sequence'][0]  # Fixed column name

a_1 = st.session_state.a_1
r = st.session_state.r
a_n = a_1 * (r ** (n - 1))  # Ensure this value is not recalculated dynamically
...
st.markdown(
    ...
    f"<span style='color: red;'>{int(a_1 * r)}</span>, "
    f"{', '.join(map(str, df_geo['Geometric Sequence'][2:5].tolist()))}..."
    f"</h3>",
...
with st.expander("You need these values for the formula:"):
    st.markdown("<h4 style='text-align: Left;'>Value of r:</h3>", unsafe_allow_html=True)
    st.latex(r"""r = {}""".format(int(r)))
    ...
    if a_1 > 0:
        ...
        text = (
            "Common Ratio: 2nd term / 1st term = "
            f"<span style='color: red;'>{a_1 * r}</span> / "
            f"(<span style='color: blue;'>{a_1}</span>)"
        )
...
with st.expander("Click here to show answer"):
    st.latex(r"""a_n = a_1 \cdot r^{{(n-1)}}""")  # Escaped curly braces for LaTeX
    st.divider()
    st.latex(r"""a_{{{}}} = {} \cdot {}^{{({}-1)}}""".format(n,a_1, int(r), n))  # Escaped curly braces for LaTeX
    st.divider()
    st.latex(r"""a_{{{}}} = {}""".format(n,a_n))  # No need to escape here
